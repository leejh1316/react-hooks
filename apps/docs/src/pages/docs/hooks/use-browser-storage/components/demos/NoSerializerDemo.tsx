...
/* ──────────────────────────────────────────────
   Demo: serializer 미적용
   기본 JSON.stringify는 Set을 "{}"로 직렬화합니다.
   세션 중에는 메모리의 Set으로 동작하는 것처럼 보이지만,
   스토리지에는 빈 객체만 저장되어 새로고침하면 즐겨찾기가
   사라지고 value는 Set이 아닌 일반 객체가 됩니다
   ────────────────────────────────────────────── */
...
const NoSerializerDemo = () => {
  const { value, setValue, removeValue } = useLocalStorage({
    ...
    // serializer 미적용 — 기본 JSON.stringify가 사용됩니다.
  });

  // 새로고침 후에는 JSON.parse("{}") 결과인 일반 객체가 들어와 Set 메서드가 없습니다.
  ...
  return (
    <div className="flex w-full flex-col items-center gap-4">
      ...
      <p className="text-caption-1 text-ink-tertiary select-none">
        즐겨찾기는 화면에 반영되지만, 스토리지에는 항상 빈 객체만 저장됩니다. 도시를 즐겨찾기하고 새로고침해 보세요.
      </p>
    </div>
  );
};
...
function FavoriteCities() {
  // ⚠ serializer 미적용 — 기본 JSON.stringify는 Set을 "{}"로 직렬화합니다.
  const { value: favorites, setValue } = useLocalStorage({
    key: "favorite-cities",
    defaultValue: new Set<string>(),
  });

  // 세션 중에는 메모리의 Set으로 동작하는 것처럼 보이지만 스토리지에는 "{}"만 저장됩니다.
  // 새로고침하면 JSON.parse("{}") 결과인 일반 객체가 들어와 즐겨찾기가 사라지고,
  // favorites.has(...)는 TypeError를 던집니다.
  const toggle = (city: string) => {
    const next = new Set(favorites);
    if (next.has(city)) next.delete(city);
    else next.add(city);
