...
/* ──────────────────────────────────────────────
   ...
   저장 후 5초가 지나면 만료됩니다. TTL 검사는 스토리지를
  ...
  return (
    <div className="flex w-full flex-col items-center gap-4">
      ...
      <p className="text-caption-1 text-ink-tertiary select-none">
        저장하고 {TTL_SECONDS}초가 지난 뒤 "다시 읽기"를 누르면 만료되어 값이 사라집니다.
      </p>
    </div>
  );
};
...
const TTLDemo = () => {
  // key를 바꿔 재마운트하면 훅이 스토리지를 다시 읽어 만료 여부를 검사합니다.
...
function TTLDemo() {
  ...
  // 만료된 값은 스토리지를 읽는 시점(마운트)에 삭제되고 defaultValue로 초기화됩니다.
  return (
    <>
      <p>저장된 값: {value ?? "(없음)"}</p>
