  ...
  return (
    <section>
      ...
      <Document.Paragraph mb={6}>
        ...
        <InlineCode>serializer</InlineCode>), 그리고 관련 타입을 named export 해요. 네 훅 모두 동일한 옵션과 반환값 구조를 가지며,
        사용하는 스토리지와 TTL 만료 지원 여부만 달라요.
      </Document.Paragraph>
      ...
      <Document.Paragraph>
        ...
        <InlineCode>dispatch</InlineCode>를 담은 객체를 반환해요.
      </Document.Paragraph>
...
const OPTION_ROWS: ParameterTableRow[] = [
  {
    name: "key",
    type: "string",
    description: "스토리지에 저장할 키예요. 같은 key를 사용하는 컴포넌트끼리는 같은 탭 안에서 자동으로 상태가 동기화돼요.",
  },
  {
    name: "defaultValue",
    type: "T",
    description: "키가 존재하지 않거나 읽기에 실패했을 때(TTL 만료, 파싱 실패, 스토리지 접근 불가 등) 사용할 기본값이에요.",
  },
  {
    name: "subscribe",
    type: "boolean",
    defaultValue: "false",
    description:
      "true이면 다른 탭에서 변경된 값이 자동 반영돼요. localStorage 훅 전용이며, 브라우저 스펙상 sessionStorage 훅에서는 효과가 없어요.",
  },
  {
    name: "serialize",
    type: "(value: T) => string",
    defaultValue: "JSON.stringify",
    description: "저장 시 값을 문자열로 변환하는 함수예요. Map, Set 등 특수 타입은 serializer 헬퍼를 사용하세요.",
  },
  {
    name: "deserialize",
    type: "(raw: string) => T",
    defaultValue: "JSON.parse",
    description: "읽을 때 문자열을 원래 값으로 되돌리는 함수예요.",
  },
  {
    name: "ttl",
    type: "number",
    description:
      "값의 유효 시간(ms)이에요. WithTTL 훅 전용이며, 생략하면 만료되지 않아요. ttl 헬퍼(ttl.hours(1) 등)로 쉽게 계산할 수 있어요.",
  },
];
...
const RETURN_ROWS: ReturnTableRow[] = [
  {
    name: "value",
    type: "T",
    description: "현재 값이에요. 마운트 시 스토리지에서 읽어온 값으로 초기화되며, 이후 setValue로 갱신돼요.",
  },
  {
    name: "setValue",
    type: "React.Dispatch<React.SetStateAction<T>>",
    description:
      "값을 갱신하고 스토리지에 저장해요. 함수형 업데이트를 지원해요. 저장에 실패하면 console.error로 기록되고 UI 상태는 낙관적으로 업데이트돼요.",
  },
  {
    name: "removeValue",
    type: "() => void",
    description: "스토리지에서 키를 삭제하고 상태를 defaultValue로 되돌려요.",
  },
  {
    name: "dispatch",
    type: "React.Dispatch<React.SetStateAction<T>>",
    description: "스토리지에 쓰지 않고 컴포넌트 상태만 갱신하는 저수준 함수예요. 일반적인 상황에서는 setValue를 사용하세요.",
  },
];
