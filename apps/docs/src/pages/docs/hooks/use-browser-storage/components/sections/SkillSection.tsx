...
const SkillSection = () => {
  return (
    <section>
      ...
      <Document.Paragraph>
        이 패키지는 Claude Code 등 AI 코딩 에이전트가 훅을 올바르게 사용하도록 돕는 SKILL 정의를 함께 제공해요. "로컬스토리지",
        "localStorage", "sessionStorage", "탭 간 동기화", "TTL", "만료 시간" 같은 키워드가 등장하면 에이전트가 이 스킬을 참고해 훅 선택
        기준, 옵션·반환값 구조, serializer / ttl 헬퍼 활용, 에러 처리 동작 등의 내용을 반영한 코드를 작성해요.
      </Document.Paragraph>
      <Document.Paragraph mb={6}>
        아래 세 파일을 각각의 경로에 그대로 복사해 저장하면 에이전트가 자동으로 인식해요. SKILL.md가 본문이고, references의 두 문서는
        에이전트가 필요할 때만 추가로 읽는 참고 문서예요.
      </Document.Paragraph>

      {/* SKILL.md */}
      <Document.Heading2>SKILL.md</Document.Heading2>
      <Document.Paragraph mb={6}>
        스킬 본문이에요. 훅 선택 기준, 공통 Options / 반환값, 사용 예시, 에러 처리 동작을 담고 있어요.
      </Document.Paragraph>
      ...
      <Document.Paragraph mb={6}>
        훅 없이 스토리지를 직접 조작하거나 <InlineCode>StorageResult</InlineCode> 타입을 다뤄야 할 때 에이전트가 참고하는 문서예요.
      </Document.Paragraph>
      ...
      <Document.Paragraph mb={6}>
        레이어 구조, 같은 탭 내 동기화 메커니즘, TTL 저장 포맷 등 내부 동작을 설명하는 문서예요.
      </Document.Paragraph>
...
description: >
  ...
  SSR 안전 처리, TTL 만료, 커스텀 직렬화, 크로스탭 동기화를 모두 지원해요.
---
...
\`useSessionStorage\`는 브라우저 스펙상 크로스탭 동기화를 지원하지 않아요.
...
- TTL 값을 ms 단위로 쉽게 계산하는 헬퍼 함수예요. 예를 들어 \`ttl.hours(1)\`은 1시간을 ms로 변환해요.
...
- 쓰기 실패 시 \`console.error\`로 에러를 기록하고, **UI 상태는 낙관적으로 업데이트**돼요. 새로고침하면 스토리지의 실제 값으로 돌아가요.
- SSR 또는 Private 모드에서는 \`UNAVAILABLE\` 에러와 함께 \`defaultValue\`가 사용돼요. 별도 처리 없이 안전하게 동작해요.
- TTL이 만료된 항목은 읽는 시점에 자동으로 삭제되고 \`defaultValue\`로 초기화돼요.
...
훅 없이 스토리지를 직접 조작할 때 사용해요. SSR과 Private 모드를 안전하게 처리해요.
...
\`storageType\`은 \`"local"\` 또는 \`"session"\`이에요.

## StorageResult 타입

모든 읽기/쓰기 함수는 \`StorageResult<T>\`를 반환해요.
...
- \`subscribe\`는 \`window storage\` 이벤트 기반으로, **값이 변경된 탭 자신에게는 발생하지 않아요.**
- SSR 환경(\`window === undefined\`)에서는 모든 함수가 \`UNAVAILABLE\` 에러를 반환하고, \`has\`는 \`false\`, \`length\`는 \`0\`, \`key\`는 \`null\`을 반환해요.
`;
...
\`useSessionStorage\`는 크로스탭 동기화가 없어요.

---

## 같은 탭 내 동기화 — useStorageState

\`useCustomEventState\`를 사용해 커스텀 이벤트 버스를 구성해요.
이벤트 키는 \`/use-browser-storage/{storageKey}\` 형식이며, **같은 탭 내에서 동일한 key를 사용하는 모든 훅 인스턴스가 자동으로 동기화**돼요.
...
\`window.addEventListener("storage", ...)\` 이벤트로 다른 탭의 변경을 감지해요.
...
> \`storage\` 이벤트는 **값이 변경된 탭 자신에게는 발생하지 않아요.**
> 때문에 같은 탭 내 동기화는 \`useCustomEventState\`가, 크로스탭 동기화는 \`storage\` 이벤트가 분리해서 담당해요.

---

## WithTTL 변형 — withTTL 래퍼

\`useBrowserStorageWithTTL\`은 \`withTTL(browserStorage)\`로 만들어진 래퍼를 getter/setter로 사용해요.
내부적으로 실제 값을 아래 포맷으로 감싸서 저장해요.
...
커스텀 \`serialize\`/\`deserialize\`는 원본 값(\`value\` 필드)에만 적용돼요. 래퍼 자체는 항상 \`JSON.stringify\`/\`JSON.parse\`로 처리돼요.

---

## getter / setter 주입 패턴

\`useStorageState\`는 실제 스토리지 종류를 알지 못해요. \`getter\`와 \`setter\`를 옵션으로 받아 상태 관리만 담당해요.
이 덕분에 일반 스토리지와 TTL 스토리지를 같은 상태 코어 위에서 구현할 수 있어요.
