  ...
  return (
    <section>
      ...
      <Document.Paragraph mb={6}>
        훅 없이 스토리지를 직접 조작할 때 사용하는 저수준 유틸 객체입니다. 모든 메서드는 예외를 던지는 대신{" "}
        <InlineCode>StorageResult</InlineCode>(성공 / 실패 유니온) 또는 안전한 기본값을 반환하므로 try-catch 없이 사용할 수 있습니다. SSR
        환경에서는 <InlineCode>UNAVAILABLE</InlineCode> 에러를 반환합니다.
      </Document.Paragraph>
      ...
      <Document.Paragraph mb={6}>
        시간 단위를 밀리초로 변환하는 헬퍼입니다. <InlineCode>WithTTL</InlineCode> 훅의 <InlineCode>ttl</InlineCode> 옵션 값을 계산할 때
        사용하세요.
      </Document.Paragraph>
...
// 다른 탭의 변경을 구독합니다. 구독 해제 함수를 반환합니다.
...
// serialize / deserialize 쌍을 반환하므로 옵션에 스프레드해 사용합니다.
useLocalStorage({
  key: "visited",
...
const BROWSER_STORAGE_ROWS: ReturnTableRow[] = [
  {
    ...
    description: "키의 값을 읽어 역직렬화해 반환합니다. 키가 없으면 NOT_FOUND 에러를 반환합니다.",
  },
  {
    name: "set",
    type: "<T>(storageType, key, value, serialize?) => StorageResult<string>",
    description: "값을 직렬화해 저장합니다. 성공 시 저장된 문자열을 반환합니다.",
  },
  {
    name: "remove",
  ...
  },
  {
    ...
    description: "해당 스토리지의 모든 항목을 삭제합니다.",
  },
  {
    name: "has",
    type: "(storageType, key) => boolean",
    description: "키의 존재 여부를 반환합니다. 접근 불가 시 false를 반환합니다.",
  },
  {
    name: "length",
    type: "(storageType) => number",
    description: "저장된 항목 개수를 반환합니다. 접근 불가 시 0을 반환합니다.",
  },
  {
    name: "key",
    type: "(storageType, index) => string | null",
    description: "인덱스에 해당하는 키를 반환합니다.",
  },
  {
    name: "keys",
    type: "(storageType) => StorageResult<string[]>",
    description: "모든 키 목록을 반환합니다.",
  },
  {
    name: "subscribe",
    type: "<T>(key, callback, deserialize?) => () => void",
    description:
      "다른 탭에서 해당 키가 변경될 때 콜백을 호출합니다. 콜백은 파싱된 값(newParsedValue / oldParsedValue)이 담긴 StorageEventExpanded를 받으며, 반환된 함수로 구독을 해제합니다. 값이 변경된 탭 자신에게는 발생하지 않습니다.",
  },
];
