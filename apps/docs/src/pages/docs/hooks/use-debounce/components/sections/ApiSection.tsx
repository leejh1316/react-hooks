  ...
  return (
    <section>
      ...
      <Document.Paragraph mb={6}>
        패키지는 <InlineCode>useDebounce</InlineCode> 훅 하나를 named export 합니다.
      </Document.Paragraph>
      ...
      <Document.Paragraph>
        <InlineCode>debounce</InlineCode>, <InlineCode>cancel</InlineCode>, <InlineCode>flush</InlineCode>를 담은 객체를 반환합니다. 세
...
const PARAMETER_ROWS: ParameterTableRow[] = [
  {
    ...
    type: "(...args: any[]) => void",
    description:
      "디바운싱할 함수입니다. 내부적으로 ref에 보관되므로 리렌더링으로 함수가 바뀌어도 항상 최신 함수가 실행됩니다.",
  },
  {
    name: "wait",
    type: "number",
    description: "지연 시간(ms)입니다. 마지막 호출 이후 이 시간 동안 추가 호출이 없을 때 함수가 실행됩니다.",
  },
  {
    name: "options",
    type: "DebounceOptions",
...
const OPTION_ROWS: ParameterTableRow[] = [
  {
    ...
    defaultValue: "true",
    description:
      "대기 중인 타이머가 없는 첫 호출 시 함수를 즉시 실행할지 여부입니다. 입력이 멈춘 뒤에만 실행하는 일반적인 디바운스 동작을 원하면 false로 지정하세요.",
  },
  {
    name: "trailing",
    type: "boolean",
    defaultValue: "true",
    description:
      "마지막 호출을 지연 시간 경과 후 실행할지 여부입니다. 실행 시에는 마지막으로 전달된 인자가 사용되며, leading으로 이미 즉시 실행된 단일 호출은 중복 실행하지 않습니다.",
  },
];
...
const RETURN_ROWS: ReturnTableRow[] = [
  {
    ...
    description:
      "디바운싱된 함수입니다. 원본 함수 대신 이 함수를 호출하세요. 인자는 원본 함수로 그대로 전달됩니다.",
  },
  {
    name: "cancel",
    type: "() => void",
  ...
  },
  {
    ...
    type: "() => void",
    description:
      "대기 중인 trailing 호출이 있다면 지연 시간을 기다리지 않고 마지막 인자로 즉시 실행한 뒤 타이머를 초기화합니다. 대기 중인 호출이 없으면 아무 동작도 하지 않습니다.",
  },
];
