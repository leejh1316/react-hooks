...
const SkillSection = () => {
  return (
    <section>
      ...
      <Document.Paragraph>
        이 패키지는 Claude Code 등 AI 코딩 에이전트가 훅을 올바르게 사용하도록 돕는 SKILL 정의를 함께 제공해요. "디바운스", "debounce",
        "입력 지연", "API 연속 호출 방지" 같은 키워드가 등장하면 에이전트가 이 스킬을 참고해 훅 시그니처, leading / trailing 옵션 조합,
        cancel·flush 활용, 스로틀과의 차이 등의 내용을 반영한 코드를 작성해요.
      </Document.Paragraph>
...
name: use-debounce
description: >
  React에서 함수 호출을 디바운싱(Debounce)할 때 사용하는 훅이에요.
...
함수 호출을 디바운싱하여 마지막 호출 이후 지정된 시간(\`wait\`) 동안 추가 호출이 없을 때만 실행되도록 하는 React 커스텀 훅이에요.
콜백을 ref에 보관해 stale closure를 방지하며, 언마운트 시 대기 중인 타이머를 자동 정리해요.
...
원본 함수 대신 반환된 \`debounce\`를 호출하세요. 인자는 원본 함수로 그대로 전달돼요.
...
- \`flush\`는 대기 중인 trailing 호출이 없으면 아무 동작도 하지 않아요.
...
이 훅은 "디바운싱된 값"이 아니라 "디바운싱된 함수"를 반환해요.
상태 값 자체를 지연시키고 싶다면 디바운싱된 함수 안에서 \`setState\`를 호출하세요.

### 2. 기본값이 leading: true예요

lodash.debounce와 달리 \`leading\` 기본값이 \`true\`이므로 첫 호출이 즉시 실행돼요.
"입력이 멈춘 뒤에만" 실행하는 일반적인 디바운스 동작을 원하면 \`{ leading: false }\`를 명시하세요.

### 3. 인자는 마지막 호출 기준

trailing 실행 시 마지막으로 전달된 인자가 사용돼요. 중간 호출의 인자는 버려져요.

### 4. wait / 옵션 변경

\`wait\`, \`leading\`, \`trailing\`이 변경되면 \`debounce\` 함수가 새로 생성돼요.
