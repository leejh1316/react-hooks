  ...
  return (
    <section>
      ...
      <Document.Paragraph mb={8}>
        ...
        이어져도 입력이 500ms 동안 멈춘 뒤에만 핸들러가 실행됩니다.
      </Document.Paragraph>

      {/* 미적용 데모 */}
      <Document.Heading2>디바운스 미적용</Document.Heading2>
      <Document.Paragraph mb={6}>
        입력창에 타이핑해 보세요. <InlineCode>change</InlineCode> 이벤트가 발생할 때마다 핸들러가 실행되어 이벤트 발생 횟수와 핸들러 실행
        횟수가 동일하게 증가합니다.
      </Document.Paragraph>
      ...
      {/* 적용 데모 */}
      <Document.Heading2>디바운스 적용</Document.Heading2>
      <Document.Paragraph mb={6}>
        같은 동작이지만 핸들러를 <InlineCode>useDebounce</InlineCode>로 감쌌습니다(<InlineCode>{`{ leading: false }`}</InlineCode>). 이벤트
        발생 횟수는 계속 증가해도 핸들러는 입력이 500ms 동안 멈춘 뒤에만 한 번 실행됩니다. <InlineCode>cancel</InlineCode> 버튼으로 대기
        중인 호출을 취소하거나, <InlineCode>flush</InlineCode> 버튼으로 기다리지 않고 즉시 실행할 수 있습니다.
      </Document.Paragraph>
      ...
      <Document.Paragraph mb={6}>
        디바운싱할 함수와 지연 시간(<InlineCode>wait</InlineCode>, ms)을 전달하면 <InlineCode>debounce</InlineCode> 함수를 반환합니다. 원본
      ...
      <Document.Heading3>
        <InlineCode>{`{ leading: false }`}</InlineCode> - 입력이 멈춘 뒤에만 실행
      </Document.Heading3>
      <Document.Paragraph mb={6}>
        첫 호출도 즉시 실행하지 않고, 호출이 멈춘 뒤 wait 시간이 지나야 마지막 호출만 실행합니다. 검색어 자동완성처럼 "입력이 끝났을 때 한
        번만" 실행하고 싶은 가장 일반적인 디바운스 패턴입니다.
      </Document.Paragraph>
      ...
      <Document.Paragraph mb={6}>
        첫 호출은 즉시 실행하고, 호출이 이어지는 동안의 나머지 호출은 모두 무시합니다. 버튼 연타 방지처럼 "즉시 반응하되 연속 호출은 막고
        싶을 때" 적합합니다.
      </Document.Paragraph>
