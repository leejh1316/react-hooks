...
const SkillSection = () => {
  return (
    <section>
      ...
      <Document.Paragraph>
        이 패키지는 Claude Code 등 AI 코딩 에이전트가 훅을 올바르게 사용하도록 돕는 SKILL 정의를 함께 제공해요. "포커스 트랩", "모달
        접근성", "Tab 순환", "키보드 내비게이션" 같은 키워드가 등장하면 에이전트가 이 스킬을 참고해 훅 시그니처, 포커스 가능 요소 판별
        기준, 중첩 트랩·Portal·지연 렌더링 등의 엣지 케이스, WAI-ARIA 접근성 체크리스트를 반영한 코드를 작성해요.
      </Document.Paragraph>
      <Document.Paragraph mb={6}>
        아래 스니펫을 그대로 복사해 프로젝트의 <InlineCode>.claude/skills/use-focus-trap/SKILL.md</InlineCode> 경로에 저장하면 에이전트가
        자동으로 인식해요.
      </Document.Paragraph>
...
/** packages/use-focus-trap/skills/use-focus-trap/SKILL.md 원문 (변경 시 함께 갱신하세요) */
const SKILL_MD_CODE = `---
name: use-focus-trap
description: >
  React에서 포커스 트랩(Focus Trap)을 구현할 때 사용하는 훅이에요.
...
특정 컨테이너(예: 모달, 다이얼로그) 내에서 Tab / Shift+Tab 포커스가 순환하도록 하는 React 커스텀 훅이에요.
DOM 변화를 MutationObserver로 감지하여 포커스 가능한 요소 목록을 동적으로 유지하며, 언마운트 시 이전 포커스를 자동 복원해요.
...
반환값은 **callback ref**예요. \`ref={containerRef}\` 형태로 컨테이너 엘리먼트에 연결하세요.
...
**1단계 셀렉터 쿼리 → 2단계 JS 필터** 두 단계로 동작해요.

### 1단계: CSS 셀렉터 (\`querySelectorAll\`)

모든 셀렉터에 \`:not([tabindex="-1"])\`가 포함되어 있어요.
...
셀렉터를 통과한 요소를 아래 조건으로 추가 필터링해요.
모두 **자신뿐 아니라 조상 요소까지** \`closest()\`로 확인해요.
...
다음 속성 변화 시 포커스 가능 요소 목록이 자동으로 갱신돼요:
...
컨테이너 내에 포커스 가능한 요소가 전혀 없으면 컨테이너 자체(\`node\`)에 포커스돼요.
컨테이너에 \`tabindex="-1"\` 또는 \`tabindex="0"\`을 추가해 두는 것을 권장해요.
...
중첩 모달 등 여러 트랩이 동시에 활성화되면 의도치 않은 동작이 발생할 수 있어요.
중첩이 필요한 경우, 부모 트랩을 먼저 비활성화(언마운트)하거나 별도의 스택 관리 로직을 추가하세요.

### 4. Portal과 함께 사용

\`createPortal\`로 렌더링된 컴포넌트도 동일하게 동작해요. \`ref\`가 실제 DOM 노드를 가리키므로 Portal 여부와 무관해요.
...
컨테이너가 마운트될 때 내부 요소가 아직 렌더링되지 않았다면, \`MutationObserver\`가 이후 추가된 요소를 자동 감지해요.
...
포커스 트랩을 올바르게 사용하려면 컴포넌트 레벨에서도 다음을 함께 구현해야 해요:
