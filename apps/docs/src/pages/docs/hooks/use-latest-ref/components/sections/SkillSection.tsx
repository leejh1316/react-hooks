...
const SkillSection = () => {
  return (
    <section>
      ...
      <Document.Paragraph>
        이 패키지는 Claude Code 등 AI 코딩 에이전트가 훅을 올바르게 사용하도록 돕는 SKILL 정의를 함께 제공해요. "stale closure", "최신 값
        참조", "의존성 배열 제거", "콜백 안정화" 같은 키워드가 등장하면 에이전트가 이 스킬을 참고해 훅 시그니처, ref 읽기 시점, props 콜백
        안정화 패턴 등의 내용을 반영한 코드를 작성해요.
      </Document.Paragraph>
...
name: use-latest-ref
description: >
  React에서 항상 최신 값을 담는 ref가 필요할 때 사용하는 훅이에요.
...
전달된 값의 최신 상태를 항상 유지하는 ref 객체를 반환하는 React 커스텀 훅이에요.
useLayoutEffect로 값이 바뀔 때마다 \`ref.current\`를 동기적으로 갱신하며, ref 객체 자체의 참조는 컴포넌트 생명주기 동안 바뀌지 않아요.
...
옵션은 없어요. 최신으로 유지할 값 하나를 전달하고, 콜백 안에서 \`ref.current\`로 읽으세요.
...
// 참조가 바뀌지 않지만 실행 시에는 항상 최신 onAction이 호출돼요.
const stableOnAction = useCallback(() => {
  latestOnActionRef.current();
}, []);
\`\`\`

memo로 감싼 자식에 \`stableOnAction\`을 전달하면 부모가 렌더링돼도 자식이 재렌더링되지 않아요.
...
\`ref.current\`는 렌더링이 끝난 뒤(useLayoutEffect 이후) 갱신돼요.
렌더링 도중 읽으면 이전 렌더링의 값이 반환될 수 있어요. 이펙트, 타이머, 이벤트 핸들러 안에서만 읽으세요.
...
상태뿐 아니라 콜백 함수도 전달할 수 있어요. 매 렌더링마다 새로 생성되는 함수를 넣어도
ref 참조는 안정적으로 유지되고, 호출 시점에는 최신 함수가 실행돼요.
