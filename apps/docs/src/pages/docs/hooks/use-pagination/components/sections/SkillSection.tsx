...
const SkillSection = () => {
  return (
    <section>
      ...
      <Document.Paragraph>
        이 패키지는 Claude Code 등 AI 코딩 에이전트가 훅을 올바르게 사용하도록 돕는 SKILL 정의를 함께 제공해요. "페이지네이션",
        "pagination", "페이지 번호", "무한 목록 페이징" 같은 키워드가 등장하면 에이전트가 이 스킬을 참고해 훅 시그니처, siblings /
        boundaries 범위 계산 규칙, 제어/비제어 모드, 접근성 체크리스트를 반영한 코드를 작성해요.
      </Document.Paragraph>
...
name: use-pagination
description: >
  React에서 페이지네이션 UI를 구축할 때 사용하는 headless 훅이에요.
...
페이지네이션 UI 구축에 필요한 모든 로직을 제공하는 headless React 훅이에요.
siblings / boundaries 설정 기반으로 렌더링할 페이지 배열을 계산하고, 생략 기호(ellipsis)를 자동 삽입하며,
제어/비제어 모드를 모두 지원해요. UI는 포함하지 않으므로 어떤 디자인에도 적용할 수 있어요.
...
- 현재 페이지가 앞쪽/뒤쪽에 가까우면 해당 방향의 ellipsis는 생략돼요. (예: 1 2 3 4 5 … 20)
- 슬롯 수(siblings × 2 + boundaries × 2 + 3)가 totalPages 이상이면 모든 페이지가 표시돼요.
...
\`paginationRange\`의 각 아이템은 고유 \`key\`를 포함해요. 배열 인덱스 대신 \`item.key\`를 사용하세요.

### 2. 제어 모드에서는 onPageChange가 필수

\`currentPage\`만 전달하고 \`onPageChange\`에서 외부 상태를 갱신하지 않으면 페이지가 바뀌지 않아요.

### 3. totalItems가 동적으로 줄어드는 경우

필터링 등으로 \`totalPages\`가 줄어 현재 페이지가 범위를 벗어나면 훅이 마지막 페이지로 자동 보정하고
\`onPageChange\`도 호출돼요. 별도의 보정 로직을 작성하지 마세요.

### 4. totalItems가 0인 경우

\`totalPages\`가 0이 되고 \`paginationRange\`는 빈 배열이 돼요.
