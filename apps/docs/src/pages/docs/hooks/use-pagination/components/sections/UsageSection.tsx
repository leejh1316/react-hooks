  ...
  return (
    <section>
      ...
      <Document.Paragraph mb={8}>
        ...
        바로 사용할 수 있는 고유 <InlineCode>key</InlineCode>를 포함해요.
      </Document.Paragraph>

      {/* 기본 사용법 데모 */}
      <Document.Heading2>기본 페이지네이션 만들기</Document.Heading2>
      <Document.Paragraph mb={6}>
        페이지 버튼과 생략 기호, 이전/다음(<InlineCode>‹</InlineCode> / <InlineCode>›</InlineCode>) 및 건너뛰기(
        <InlineCode>«</InlineCode> / <InlineCode>»</InlineCode>) 버튼을 모두 사용하는 예시예요. <InlineCode>isFirstPage</InlineCode> /{" "}
        <InlineCode>isLastPage</InlineCode>를 사용해서 양 끝에서 버튼을 비활성화할 수 있어요.
      </Document.Paragraph>
      ...
      {/* 옵션 조정 플레이그라운드 데모 */}
      <Document.Heading2>옵션 조정하기</Document.Heading2>
      <Document.Paragraph mb={6}>
        ...
        표시할 페이지 수예요. 아래 데모에서 네 값을 슬라이더로 바꿔가며 <InlineCode>paginationRange</InlineCode>가 어떻게 달라지는지
      ...
      {/* 건너뛰기 내비게이션 */}
      <Document.Heading2>여러 페이지 건너뛰기</Document.Heading2>
      <Document.Paragraph mb={6}>
        <InlineCode>handleSkipNext</InlineCode> / <InlineCode>handleSkipPrevious</InlineCode> 메서드를 사용해서 한 번에{" "}
        <InlineCode>siblings × 2 + 1</InlineCode> 페이지씩 이동할 수 있어요. 현재 보이는 페이지 묶음(siblings 윈도우)이 통째로 넘어가는
        이동 폭이며, 범위를 벗어나면 첫/마지막 페이지로 보정돼요.
      </Document.Paragraph>
      <CodeBlock code={SKIP_NAVIGATION_CODE} className="mb-8" />

      {/* 제어 모드 */}
      <Document.Heading2>외부 상태로 페이지 제어하기</Document.Heading2>
      <Document.Paragraph mb={6}>
        <InlineCode>currentPage</InlineCode>와 <InlineCode>onPageChange</InlineCode> 속성을 사용해서 페이지 상태를 외부에서 제어할 수 있어요.
        URL 쿼리 파라미터와 동기화하거나, 페이지 변경 시 서버에 데이터를 요청하는 서버 사이드 페이지네이션에 유용해요.
      </Document.Paragraph>
