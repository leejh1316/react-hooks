  ...
  return (
    <section>
      ...
      <Document.Paragraph mb={6}>
        패키지는 <InlineCode>useThrottle</InlineCode> 훅 하나를 named export 해요.
      </Document.Paragraph>
      ...
      <Document.Paragraph>
        <InlineCode>throttle</InlineCode>, <InlineCode>cancel</InlineCode>, <InlineCode>flush</InlineCode>를 담은 객체를 반환해요. 세
...
const PARAMETER_ROWS: ParameterTableRow[] = [
  {
    ...
    type: "(...args: any[]) => void",
    description:
      "스로틀링할 함수예요. 내부적으로 ref에 보관되므로 리렌더링으로 함수가 바뀌어도 항상 최신 함수가 실행돼요.",
  },
  {
    name: "wait",
    type: "number",
    description: "실행 간격 제한 시간(ms)이에요. 이 시간당 최대 한 번만 함수가 실행돼요.",
  },
  {
    name: "options",
    type: "ThrottleOptions",
...
const OPTION_ROWS: ParameterTableRow[] = [
  {
    ...
    type: "boolean",
    defaultValue: "true",
    description: "첫 호출 시 함수를 즉시 실행할지 여부예요. false이면 첫 호출도 제한 시간 이후로 미뤄져요.",
  },
  {
    name: "trailing",
    type: "boolean",
    defaultValue: "true",
    description:
      "제한 시간 동안 발생한 마지막 호출을 제한 시간 경과 후 실행할지 여부예요. 실행 시에는 마지막으로 전달된 인자가 사용돼요.",
  },
];
...
const RETURN_ROWS: ReturnTableRow[] = [
  {
    ...
    description:
      "스로틀링된 함수예요. 원본 함수 대신 이 함수를 호출하세요. 인자는 원본 함수로 그대로 전달돼요.",
  },
  {
    name: "cancel",
    type: "() => void",
  ...
  },
  {
    ...
    type: "() => void",
    description:
      "대기 중인 trailing 호출이 있다면 제한 시간을 기다리지 않고 마지막 인자로 즉시 실행한 뒤 타이머를 초기화해요. 대기 중인 호출이 없으면 아무 동작도 하지 않아요.",
  },
];
