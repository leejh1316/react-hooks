...
const SkillSection = () => {
  return (
    <section>
      ...
      <Document.Paragraph>
        이 패키지는 Claude Code 등 AI 코딩 에이전트가 훅을 올바르게 사용하도록 돕는 SKILL 정의를 함께 제공해요. "스로틀", "throttle",
        "이벤트 성능 최적화", "호출 빈도 제한" 같은 키워드가 등장하면 에이전트가 이 스킬을 참고해 훅 시그니처, leading / trailing 옵션
        조합, cancel·flush 활용, 디바운스와의 차이 등의 내용을 반영한 코드를 작성해요.
      </Document.Paragraph>
...
name: use-throttle
description: >
  React에서 함수 호출을 스로틀링(Throttle)할 때 사용하는 훅이에요.
...
함수 호출을 스로틀링하여 지정된 시간(\`wait\`)당 최대 한 번만 실행되도록 하는 React 커스텀 훅이에요.
...
원본 함수 대신 반환된 \`throttle\`을 호출하세요. 인자는 원본 함수로 그대로 전달돼요.
...
이 훅은 "스로틀링된 값"이 아니라 "스로틀링된 함수"를 반환해요.
상태 값 자체를 지연시키고 싶다면 스로틀링된 함수 안에서 \`setState\`를 호출하세요.

### 2. 인자는 마지막 호출 기준

trailing 실행 시 제한 시간 동안 마지막으로 전달된 인자가 사용돼요. 중간 호출의 인자는 버려져요.

### 3. wait / 옵션 변경

\`wait\`, \`leading\`, \`trailing\`이 변경되면 \`throttle\` 함수가 새로 생성돼요.
