  ...
  return (
    <section>
      ...
      <Document.Paragraph mb={8}>
        ...
        아무리 자주 발생해도 핸들러가 500ms당 최대 한 번만 실행돼요.
      </Document.Paragraph>

      {/* 미적용 데모 */}
      <Document.Heading2>스로틀 없이 사용하기</Document.Heading2>
      <Document.Paragraph mb={6}>
        스로틀을 적용하지 않았을 때의 동작을 보여주는 예시예요. 영역 안에서 마우스를 움직여 보세요. <InlineCode>mousemove</InlineCode>{" "}
        이벤트가 발생할 때마다 핸들러가 실행되어 이벤트 발생 횟수와 핸들러 실행 횟수가 동일하게 증가해요.
      </Document.Paragraph>
      ...
      {/* 적용 데모 */}
      <Document.Heading2>스로틀 적용하기</Document.Heading2>
      <Document.Paragraph mb={6}>
        같은 핸들러를 <InlineCode>useThrottle</InlineCode>로 감싼 예시예요. 이벤트 발생 횟수는 계속 증가해도 핸들러 실행 횟수는
        500ms당 한 번씩만 늘어나요. <InlineCode>cancel</InlineCode> 버튼으로 대기 중인 trailing 호출을 취소하거나,{" "}
        <InlineCode>flush</InlineCode> 버튼으로 기다리지 않고 즉시 실행할 수 있어요.
      </Document.Paragraph>
      ...
      {/* 기본 사용법 */}
      <Document.Heading2>함수 스로틀링하기</Document.Heading2>
      <Document.Paragraph mb={6}>
        가장 단순한 사용 예시예요. 스로틀링할 함수와 제한 시간(<InlineCode>wait</InlineCode>, ms)을 전달하면 <InlineCode>throttle</InlineCode> 함수를 반환해요. 원본
      ...
      <Document.Paragraph mb={6}>
        첫 호출은 즉시 실행하고, 제한 시간 동안 발생한 나머지 호출은 모두 무시해요. 버튼 연타 방지처럼 "즉시 반응하되 반복은 막고 싶을 때"
        적합해요.
      </Document.Paragraph>
      <CodeBlock code={LEADING_ONLY_CODE} className="mb-8" />

      <Document.Heading3>
        <InlineCode>{`{ leading: false }`}</InlineCode> - 마지막 호출만 지연 실행하기
      </Document.Heading3>
      <Document.Paragraph mb={6}>
        첫 호출을 즉시 실행하지 않고, 제한 시간이 지난 뒤 마지막 호출만 실행해요. 스크롤이 진행되는 동안이 아니라 일정 간격으로 최종
        상태를 반영하고 싶을 때 적합해요.
      </Document.Paragraph>
...
function ScrollTracker() {
  ...
  // scrollY 반영이 200ms당 최대 한 번으로 제한돼요.
...
// 첫 클릭은 즉시 실행되고, 이후 1초 동안의 클릭은 모두 무시돼요.
...
// 즉시 실행되지 않고, 200ms가 지난 뒤 마지막 호출의 인자로 실행돼요.
